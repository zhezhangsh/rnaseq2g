rnaseq2g.plot.meta <- function(res, typ, nm, top) { 
  if (!is.null(res)) {
    if (nm=='') plot(0, type='n', axes=FALSE, xlab='', ylab='') else {
      p1 <- res[, 3];  
      p2 <- res[, nm];
      p1[is.na(p1)] <- 1;
      p2[is.na(p2)] <- 1;
      ...
      if (typ == '1') {
        x  <- sort(-log10(p1));
        y  <- sort(-log10(p2)); 
        lb <- paste('-Log10(p value)', c('Meta-analysis', nm), sep=', ');
        ...
        rk1 <- rank(p1);
        rk2 <- rank(p2);
        lb  <- paste('P value ranking', c('Meta-analysis', nm), sep=', ');
        len <- length(rk1); 
        ...
        uni <- names(p1); 
        PlotVenn(s1, s2, c('Meta-analysis', nm), uni, FALSE, title = 'Top gene overlapping', cex = 2);
      }
    }
